...
-   There are 603 cases in the dataset.

-   There are a variety types of data in the dataset (e.g. double,
    characters, etc)
...
-   Each row represent a case or a student in this dataset.

What columns typically (or you think may) represent:

-   Each column represents a variable or student information in this
    dataset.
...
-   Only the rows associated with students who earned a final grade (as
    a percentage) of 80% or higher are displayed.
...
```{r}
d |> 
  filter(percentage_earned > 0.80) |>
  select(student_id, total_points_earned) |> 
